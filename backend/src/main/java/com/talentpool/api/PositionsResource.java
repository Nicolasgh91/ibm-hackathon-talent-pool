...
public class PositionsResource {
  ...
  public Response create(@Valid CreatePuestoRequest request, @Context SecurityContext securityContext) {
  ...
  public Response ranking(@PathParam("id") UUID puestoId) {
    ...
    return Response.ok(new RankingResponse(puesto.id, puesto.titulo, ranking.size(), ranking)).build();
  }
}
