 ...
 * guardrails (max 2000 chars, injection detection) - Token usage tracking - Structured logging with
 * correlation IDs
 */
@Path("/api/v1/chat")
...
public class ChatResource {
   ...
   *
   * <p>Acceptance Criteria: - User must be authenticated (JWT) - Message max 2000 characters - Rate
   * limited to 10 requests/minute - Returns response with token usage - Latency p95 < 8s
   *
  ...
  public Response health() {
    ...
    return Response.ok().entity(java.util.Map.of("status", "healthy", "service", "chat")).build();
  }
}
